...
{
	public class nightstring : ModItem
	{
		public override void SetStaticDefaults() {
			DisplayName.SetDefault("Night String");
		...
		public override void SetDefaults() {
			...
			item.value = 400000; // how much the item sells for (measured in copper)
		...
		public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
		{
			...
			for (int i = 0; i < numberProjectiles; i++)
			{
				Vector2 perturbedSpeed = new Vector2(speedX, speedY).RotatedBy(MathHelper.Lerp(-rotation, rotation, i / numberProjectiles)) * 16f; // Watch out for dividing by 0 if there is only 1 projectile.
				Projectile.NewProjectile(position.X, position.Y, perturbedSpeed.X, perturbedSpeed.Y, type, damage, knockBack, player.whoAmI);
			}
			return false;
		}
